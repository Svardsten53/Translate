...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['svensk'])) {
    ...
    echo "--- Startar uppdateringsskriptet ---\n\n";

    // Kör kommandot och fånga utdata med rätt användarbehörighet
    // 2>&1 skickar felmeddelanden till samma utdata som vanlig text.
    $command = 'cd ' . $home . '/BirdNET-Pi && sudo -u ' . $user . ' wget -O uppgradering.sh https://raw.githubusercontent.com/Svardsten53/Translate/main/sweden/uppgradering.sh && sudo -u ' . $user . ' chmod u+x uppgradering.sh && sudo -u ' . $user . ' dos2unix uppgradering.sh && sudo -u ' . $user . ' ./uppgradering.sh 2>&1';
...
<div class="systemcontrols">
  ...
  <button id="svenskUppdateringButton">
    > > > Uppdatering med svensk anpassning < < < 
...
<div style="text-align: center;">
  Klicka på "Update" för att när som helst gå tillbaks till engelsk originalversion
</div>
<br>
...
<script>
    document.getElementById('svenskUppdateringButton').addEventListener('click', function() {
        if (confirm('Vill du köra uppdatering med svensk anpassning?')) {
            ...
            resultatOutput.textContent = 'Uppdatering pågår, vänligen vänta...';
