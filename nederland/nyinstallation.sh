...
wget -O Translate.zip "https://raw.githubusercontent.com/RonnyCHL/Translate/main/nederland/Translate.zip" && unzip -o Translate.zip && rm Translate.zip
...
# Adapt scripts
cd Translate/skript
sudo sed -i "s|/home/userid-att-andra|$HOME|g" 2_backuporg.sh 5_kopieraegna.sh 3_restoreorg.sh 6_backupegna.sh 7_translate.sh 8_restoreegna.sh
echo "Current system user updated in scripts"

# Write location to index.php
echo 'export PLATS="Schrijf hier je locatie' >> ~/.bashrc && source ~/.bashrc
...
sudo ./7_translate.sh && echo "Translation and adaptation to Dutch done"
...
cd ~/BirdNET-Pi
echo "* * * * * Nieuwe installatie aanpassing voltooid! / New installation modification complete! * * * * *"
